...
def main(start, end, dpi, image_size, module_width):
    if end == -1:
        end = start
    ...
    for code in range(start, end+1):
        ...
        barcode_image = _render_barcode(width, height, code, dpi, module_width)
...
def _render_barcode(max_width: int, max_height: int, code: int, dpi: int, module_width: Optional[float]):
    if module_width:
        image = _render_barcode_with_size(code, dpi, module_width)
    ...
    for size in reversed(sorted(SIZES.keys())):  # try size from big to small until it fits
        image = _render_barcode_with_size(code, dpi, SIZES[size])
        width, height = image.size
        if width <= max_width and height <= max_height:
            return image
...
def _render_barcode_with_size(code: int, dpi: int, module_width: float):
    code_str = f'{code:012}'
    ean = barcode.get('ean13', code_str, writer=ImageWriter())
    ean.writer.dpi = dpi
    image = ean.render(writer_options={'module_width': module_width}, text=f'{code}')
    return image
